...
app = FastAPI(title="survey-district-fit-service", version="1.0.0")
...
class SurveyDistrictRequest(BaseModel):
    survey_id: str = "survey-000"
    top_k: int = Field(default=5, ge=1, le=20)
    preferred_category: str | None = None
    risk_tolerance: str | None = None
    budget_million_krw: float | None = None
    rent_sensitivity: float | None = None
    footfall_preference: float | None = None
    resident_preference: float | None = None
    worker_preference: float | None = None
    competition_tolerance: float | None = None
...
def catalog() -> dict[str, Any]:
    return {"surveys": _metadata["surveys"], "districts": _metadata["districts"]}


def _survey(request: SurveyDistrictRequest) -> dict[str, Any]:
...
@app.post("/predict")
def predict(request: SurveyDistrictRequest) -> dict[str, Any]:
    survey = _survey(request)
    items = _metadata["districts"]
