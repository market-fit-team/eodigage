...
import { HitlDecisionToolbar } from "@/features/chat/components/hitl/hitl-decision-toolbar"
import { HitlEditedActionEditor } from "@/features/chat/components/hitl/hitl-edited-action-editor"
import { useHitlDecisionForm } from "@/features/chat/hooks/use-hitl-decision-form"
import type { HitlDecisionFormValues } from "@/features/chat/lib/hitl-decisions/build-resume-decisions-from-form"
import type {
  HitlDecisionDraft,
  HitlDecisionDraftMap,
} from "@/features/chat/types/hitl-decision-draft"
import type {
  HitlActionRequest,
  HitlDecision,
  HitlDecisionType,
...
function HitlInterruptForm({
  ...
  return (
    <div className="w-full rounded-xl border border-amber-300/60 bg-amber-50/70 text-amber-950 shadow-sm">
      <div className="flex items-center gap-2 border-b border-amber-200/80 px-4 py-3">
        <ShieldAlert className="size-4 text-amber-700" />
        <span className="text-sm font-medium">도구 실행 승인이 필요합니다</span>
      </div>
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-background p-4">
      <div className="space-y-1">
        <div className="font-mono text-sm font-semibold">{request.name}</div>
      ...
      <ScrollArea className="mt-3 h-36 rounded-lg border border-border/70 bg-muted/40 p-3">
