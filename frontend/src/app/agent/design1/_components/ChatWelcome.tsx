...
export function ChatWelcome({ onSelectPrompt }: ChatWelcomeProps) {
  const promptSuggestions = [
    ...
    },
    {
      ...
      prompt:
        "이 프로젝트의 UI 컴포넌트를 활용해 미니멀하게 코딩하는 팁 알려줘.",
    },
  ]
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-1 flex-col items-center justify-center p-6 text-center">
      {/* 로고 영역 */}
      <div className="mb-6 flex size-12 items-center justify-center rounded-2xl border border-border/20 bg-muted/40">
      ...
      <p className="mt-2 max-w-sm text-xs text-muted-foreground">
        이곳은 작동하는 가상의 미니멀리즘 채팅 에이전트 공간입니다. 아래 추천
        제안을 선택해 대화를 시작해 보세요.
      </p>
      ...
      <div className="mt-10 grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
        {promptSuggestions.map((item, idx) => (
          <button
            ...
            className="flex flex-col items-start rounded-xl border border-border/40 bg-muted/5 p-4 text-left transition-all hover:border-border/80 hover:bg-muted/40 focus:outline-none"
          >
            <div className="mb-2 flex size-7 items-center justify-center rounded-lg border border-border/20 bg-background">
              {item.icon}
            </div>
            <span className="text-xs font-semibold text-foreground/80">
              {item.title}
            </span>
            <span className="mt-1 text-[11px] leading-normal text-muted-foreground/80">
              {item.desc}
            </span>
          </button>
        ))}
