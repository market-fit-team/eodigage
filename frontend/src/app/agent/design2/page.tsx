...
import { 
  Plus, MessageSquare, Settings, Menu, ChevronLeft, ChevronRight,
  ShieldCheck, LayoutDashboard, BrainCircuit
} from "lucide-react"
...
export default function Page() {
  ...
  const [activeArtifactId, setActiveArtifactId] = React.useState<string | null>("art-dash")
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background text-foreground text-xs font-light">
      
      {/* 1. 좌측 메인 룸 사이드바 */}
      <aside 
        className={cn(
          "h-full border-r border-border/40 bg-muted/10 transition-all duration-300 flex flex-col shrink-0 overflow-hidden",
      ...
      >
        ...
        <div className="p-3 shrink-0">
          <Button 
            variant="outline" 
            size="sm" 
            className="w-full flex items-center justify-center gap-1.5 text-[11px] font-normal cursor-pointer"
        ...
        <div className="flex-1 overflow-y-auto px-2 py-1 space-y-1">
          ...
          {rooms.map((room) => (
            <button
              ...
              className={cn(
                "w-full text-left rounded-lg p-2.5 transition-colors flex items-start gap-2.5 select-none cursor-pointer",
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <span className="text-[8px] text-muted-foreground/60">{room.updatedAt}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        {/* 사이드바 하단 설정 */}
        <div className="p-3 border-t border-border/20 bg-muted/5 shrink-0 flex items-center justify-between text-muted-foreground">
      ...
      {!isSidebarOpen && (
        <button 
          ...
          className="absolute left-4 top-4 z-50 rounded-lg border border-border/50 bg-background p-1.5 shadow-md hover:bg-muted transition-colors cursor-pointer"
        >
      ...
      <main className="flex-1 h-full min-w-0 relative">
        <ResizablePanelGroup orientation="horizontal" className="h-full w-full">
          ...
          {/* 분할 드래그 핸들 */}
          <ResizableHandle withHandle className="bg-border/20 hover:bg-primary/40 transition-colors w-1.5" />
